...
export function demoExperimentPlan(hypothesis: string, qc: LiteratureQC | null): ExperimentPlan {
  ...
  return {
    ...
    executive_summary: section(
      `Demo data: this plan frames the hypothesis as a source-grounded ${domainText} study with budget estimates, a timeline, safety review gates, and traceable source grounding.`,
      [firstUrl],
      0.7,
    ),
    budget: section([
      { category: "Materials", item: key === "solar" ? "Candidate absorber material set" : key === "hela" ? "Authenticated cell model source" : "Paper sensor substrate and capture chemistry", estimated_cost: key === "solar" ? 1200 : 450, notes: `Demo estimate grounded by ${firstUrl}.` },
      { category: "Reagents & Assays", item: key === "solar" ? "Device characterization access" : key === "hela" ? "Viability and membrane integrity assay" : "Electrochemical reader or shared instrumentation", estimated_cost: key === "solar" ? 1800 : 850, notes: `Demo estimate grounded by ${validationUrl}.` },
      { category: "Review & Analysis", item: "PI review, safety review, and analysis time", estimated_cost: 500, notes: "Planning estimate for hackathon demo; replace with local quotes." },
